...
<head>
  ...
  <style>
	...
	*{box-sizing:border-box;font-family:"Microsoft YaHei",sans-serif;margin:0;padding:0}
	...
	/* 核心修改：纯正行楷+日间/夜间分色渐变，夜间清晰不偏色 */
	.logo{
		font-weight:700;
		font-size:28px;
		/* 强制纯正行楷字体，多库兜底不回退 */
		font-family:"STKaiti","KaiTi","楷体_GB2312","AR PL UKai CN",serif;
		letter-spacing:2px;
		/* 日间模式：红黑渐变 */
...
<div class="head-bar">
  <div class="logo">征天写作</div>
  <select id="projectSelector" class="project-selector" onchange="switchProject()"></select>
  <button class="func-btn project-set-btn" id="delProjectBtn" onclick="showDelModal()">⚙️ 删除项目</button>
  <button class="func-btn" onclick="newProjectModal()">📁 新建</button>
  <button class="func-btn" onclick="toggleSet()">⚙️ 设置</button>
  <button class="func-btn" id="themeBtn">☀️ 日间</button>
</div>

<!-- 删除项目弹窗 -->
<div class="modal" id="delModal"><div class="modal-content">
  <div class="step-title"><span>🗑️</span> 删除项目</div>
  <p style="margin-bottom:15px;">删除后所有内容永久消失，不可恢复！</p>
  <div class="btn-group">
	<button class="btn btn-light" onclick="hideDelModal()">取消</button>
	<button class="btn btn-danger" onclick="deleteProjectForever()">确认删除</button>
  </div>
</div></div>

<!-- 新建弹窗 -->
<div class="modal" id="newModal"><div class="modal-content">
  <div class="step-title"><span>📚</span> 新建项目</div>
  <div class="form-item"><input type="text" id="newName" placeholder="输入项目名"></div>
  <div class="btn-group">
	<button class="btn btn-light" onclick="hideNewModal()">取消</button>
	<button class="btn btn-primary" onclick="createCleanProject()">创建</button>
  </div>
</div></div>

<!-- 设置弹窗 -->
<div class="modal" id="setModal"><div class="modal-content">
  <div class="step-title"><span>🔑</span> API设置</div>
  <div class="form-item"><input type="text" id="apiKey" placeholder="API Key（sk-xxxx）"></div>
  <div class="form-item"><input type="text" id="model" value="deepseek-chat" placeholder="模型"></div>
  <div class="form-item"><input type="number" id="temp" value="0.7" step="0.1" min="0.1" max="1.0" placeholder="温度(0.1-1.0)"></div>
  <div class="form-item"><input type="number" id="maxTokens" value="2048" placeholder="最大Tokens"></div>
  <div class="btn-group"><button class="btn btn-primary" onclick="saveApi()">保存设置</button></div>
</div></div>

<div class="main">
  <div class="step-nav-wrap">
	<button class="back-btn" id="backBtn" disabled onclick="goPrevStep()">← 返回</button>
	<div class="step-nav">
	  <div class="nav" data-step="1" onclick="goStep(1)">1 故事核</div>
	  <div class="nav" data-step="2" onclick="goStep(2)">2 大纲</div>
	  <div class="nav" data-step="3" onclick="goStep(3)">3 细纲</div>
	  <div class="nav" data-step="4" onclick="goStep(4)">4 章纲</div>
	  <div class="nav" data-step="5" onclick="goStep(5)">5 正文</div>
	  <div class="nav" data-step="6" onclick="goStep(6)">6 AI修改(选填)</div>
	</div>
  </div>

  <!-- 1故事核 → 视角+风格选择 -->
  <div id="step1" class="step-page">
	<div class="step-box"><div class="step-title"><span>1</span>灵感/关键词 + 创作设定</div>
	  <div class="form-item"><textarea id="idea" placeholder="输入灵感、关键词或初步设定" oninput="saveNow()"></textarea></div>
	  <div class="style-row">
		<div class="style-col">
		  <label class="label">叙事视角</label>
		  <select id="viewSelect" onchange="saveNow()">
			<option value="第三人称">第三人称（默认）</option>
		...
		<div class="style-col">
		  ...
		  <select id="styleSelect" onchange="saveNow()">
			<option value="小白直白爽文">小白直白爽文（默认）</option>
			<option value="都市写实">都市写实</option>
			<option value="古风细腻">古风细腻</option>
			<option value="悬疑紧凑">悬疑紧凑</option>
			<option value="校园清新">校园清新</option>
		  </select>
		</div>
	  </div>
	  <button class="btn btn-primary" onclick="genCore()" style="margin-top:12px;">生成故事核</button>
	  <div id="coreLoading" class="loading hide">正在生成故事核...</div>
	  <div id="coreGrid" class="card-grid hide"></div>
	  <button class="btn btn-success hide" id="next1" onclick="goStep(2)">下一步</button>
	</div>
  </div>

  <!-- 2大纲 → 目标总字数 -->
  <div id="step2" class="step-page hide">
	<div class="step-box"><div class="step-title"><span>2</span>逆推大纲</div>
	  <div class="form-item">
		<label class="label">目标总字数（万字）</label>
	  ...
	  <button class="btn btn-primary" onclick="genOutline()">生成大纲</button>
	  <div id="outlineLoading" class="loading hide">正在生成大纲...</div>
	  <div class="form-item">
		<label class="label">大纲内容</label>
		<textarea id="outline" rows="8" oninput="saveNow()"></textarea>
	  </div>
	  <button class="btn btn-success" onclick="goStep(3)">下一步</button>
	</div>
  </div>

  <!-- 3细纲 → 节点控制 -->
  <div id="step3" class="step-page hide">
	<div class="step-box"><div class="step-title"><span>3</span>细纲</div>
	  <button class="btn btn-primary" onclick="genDetail()">生成细纲</button>
	  <div id="detailLoading" class="loading hide">正在生成细纲...</div>
	  <div class="form-item">
		<label class="label">细纲内容</label>
		<textarea id="detail" rows="10" oninput="saveNow()"></textarea>
	  </div>
	  <button class="btn btn-success" onclick="goStep(4)">下一步</button>
	</div>
  </div>

  <!-- 4章纲 → 字数适配 -->
  <div id="step4" class="step-page hide">
	<div class="step-box"><div class="step-title"><span>4</span>章纲</div>
	  <button class="btn btn-primary" onclick="genZhang()">生成章纲</button>
	  <div id="zhangLoading" class="loading hide">正在生成章纲...</div>
	  <div class="form-item">
		<label class="label">章纲内容</label>
		<textarea id="zhang" rows="12" oninput="saveNow()"></textarea>
	  </div>
	  <button class="btn btn-success" onclick="goStep(5)">下一步</button>
	</div>
  </div>

  <!-- 5正文 → 严格遵循章纲 + 修复一键复制 -->
  <div id="step5" class="step-page hide">
	<div class="step-box"><div class="step-title"><span>5</span>正文生成</div>
	  <div class="form-item">
		<label class="label">章节号</label>
		<input type="number" id="chapNum" value="1" min="1" step="1">
	  </div>
	  <button class="btn btn-primary" onclick="writeChap()">生成章节</button>
	  <div id="chapLoading" class="loading hide">正在生成正文...</div>
	  <div class="btn-group">
		<button class="btn btn-light" onclick="copyAllText()">✅ 一键复制全文</button>
		<button class="btn btn-light" onclick="exportTxt()" style="display:none;">📄 导出TXT</button>
	  </div>
	  <div class="form-item">
		<label class="label">正文内容</label>
		<div id="resultWrite" class="result"></div>
	  </div>
	  <button class="btn btn-light" onclick="goStep(6)" style="margin-top:10px;">→ 进入AI修改</button>
	</div>
  </div>

  <!-- 6AI修改 → 保持风格一致 -->
  <div id="step6" class="step-page hide">
	<div class="step-box"><div class="step-title"><span>6</span>AI修改(选填)</div>
	  <div class="form-item">
		<label class="label">选择章节</label>
		<select id="editChap"></select>
	  </div>
	  <div class="form-item">
		<label class="label">修改建议</label>
		<textarea id="editTip" placeholder="输入修改建议：如增加情绪、调整节奏、局部改写"></textarea>
	  </div>
	  <button class="btn btn-primary" onclick="aiEdit()">提交修改</button>
	  <div id="editLoading" class="loading hide">正在修改章节...</div>
	  <button class="btn btn-light" onclick="goStep(5)" style="margin-top:10px;">← 返回正文</button>
	</div>
  </div>
</div>
...
function renderAll() {
  ...
  document.getElementById("viewSelect").value = BOOK.view || "第三人称";
  document.getElementById("styleSelect").value = BOOK.style || "小白直白爽文";
  ...
  BOOK.contents.forEach(c => {
	...
	div.innerHTML = `<h3>第${c.num}章</h3><p>${c.content}</p>`;
	res.appendChild(div);
  });
  
  let sel = document.getElementById("editChap");
  sel.innerHTML = '<option value="">选章节</option>';
  BOOK.contents.forEach(c => {
	let opt = document.createElement("option");
	opt.value = c.num;
	opt.textContent = `第${c.num}章`;
	sel.appendChild(opt);
  });
}
...
function updateChapterSuggestion() {
  ...
  const suggestionText = `💡 建议章纲数：${minChapters}-${maxChapters} 章（每章约${avgWordsPerChapter}字，目标总字数约${totalWords}字）`;
...
function copyAllText() {
  ...
  if (!text.trim()) return alert("暂无正文可复制！");
  
  if (navigator.clipboard && window.isSecureContext) {
	navigator.clipboard.writeText(text).then(() => {
	  alert("✅ 全文复制成功！");
	}).catch(err => {
	  console.error("复制失败（现代浏览器）：", err);
	  fallbackCopy(text);
	});
  } else {
	fallbackCopy(text);
...
function exportTxt() {
  ...
  if (!text.trim()) return alert("暂无正文可导出！");
  let blob = new Blob([text], { type: "text/plain;charset=utf-8" });
  let url = URL.createObjectURL(blob);
  let a = document.createElement("a");
  a.href = url;
  a.download = "征天写作_" + (currentId ? globalData.projects[currentId].name : "正文") + ".txt";
  a.click();
...
async function aiRequest(system, user) {
  ...
  try {
	...
	if (!res.ok) {
	  ...
	  throw new Error(`API请求失败（状态码：${res.status}）：${errorData.message || "网络异常"}`);
	}
...
async function genCore() {
  ...
  if (!idea) return alert("请输入灵感/关键词");
...
async function genOutline() {
  if (!BOOK.core) return alert("请先选择故事核");
  ...
  try {
	const result = await aiRequest(
	  `你是网络小说大纲写手，写作视角为${view}，风格为${style}；用通俗直白的语言，分3-5个阶段，每个阶段聚焦核心情节推进，不用生僻比喻和过度修饰，符合网络小说节奏，无AI味。`,
	  `根据故事核生成${totalWan}万字的${style}风格${view}小说大纲，每个阶段说明核心冲突和情节走向：${BOOK.core}`
	);
...
async function genDetail() {
  if (!BOOK.outline.trim()) return alert("请先填写大纲");
  const view = BOOK.view;
  const style = BOOK.style;
  ...
  try {
	const result = await aiRequest(
	  `你是网络小说细纲写手，写作视角为${view}，风格为${style}；拆解为关键节点，每个阶段节点不超过3个，节点是核心转折点/信息揭露/角色抉择，每节点不超50字，逻辑递进，无AI味。`,
	  `将以下大纲拆解为${style}风格${view}细纲，严格遵守要求：\n\n${BOOK.outline}`
	);
...
async function genZhang() {
  if (!BOOK.detail.trim()) return alert("请先填写细纲");
  ...
  try {
	const result = await aiRequest(
	  `你是网络小说章纲写手，写作视角为${view}，风格为${style}；总字数约${totalWords}字，建议生成${suggestedChapters}章，每章约${avgWordsPerChapter}字；严格基于细纲，每章以"**第X章 标题**"开头，核心内容不超80字，清晰明了。`,
	  `根据以下细纲生成${style}风格${view}章纲，严格遵守要求：\n\n${BOOK.detail}`
	);
...
async function writeChap() {
  if (!BOOK.zhang.trim()) return alert("请先填写章纲");
  ...
  try {
	const result = await aiRequest(
	  `你是网络小说正文写手，**严格遵循章纲创作，不得新增未提及的情节/设定/人物**；写作视角为${view}，风格为${style}；用日常常见的比喻，减少过度华丽修饰，聚焦情节推进和角色动作，对话符合风格/视角，读起来像真人手写，不用改可直接发布，每章约1500字。`,
	  `根据以下章纲生成第${chapNum}章${style}风格${view}正文，严格遵守要求：\n\n${BOOK.zhang}`
	);
...
async function aiEdit() {
  ...
  const view = BOOK.view;
  const style = BOOK.style;
  if (!chap || !tip) return alert("请选择章节并填写修改建议");
  ...
  try {
	const result = await aiRequest(
	  `你是网络小说修改助手，保持写作视角为${view}、风格为${style}；用常见易懂的比喻、减少过度修饰、语言通俗流畅，无AI味，改完不用二次调整；严格按修改建议改，不偏离原情节。`,
	  `修改以下章节内容（要求：${tip}），保持${view}/${style}风格：\n原内容：${chap.content}`
	);
	chap.content = result;
	renderAll();
	saveNow();
	alert(`第${chapNum}章修改成功！`);
  } catch (e) {
